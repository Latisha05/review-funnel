...
export function getDynamicQrUrl(env, qrCodeId = "") {
  ...
  const baseUrl = (env.APP_BASE_URL || "https://review-funnel.pages.dev").replace(/\/$/, "");
...
export async function firestorePatch(env, docPath, data) {
  ...
  const url = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/${docPath}?${fieldPaths}`;
  const res = await fetch(url, {
    method: "PATCH",
    headers: { Authorization: `Bearer ${await getAccessToken(env)}`, "Content-Type": "application/json" },
    body: JSON.stringify(toFirestoreDoc(data)),
